...
o3video = function( opts, container ) {
	...
	self.constructor__ = function() {
		...
		if ( src != '' ) {
			...
			if ( src != '' && ext2mime(src) != '' )
				self.source.push( { src: src, type: ext2mime(src), has_codec: is_codec_video(ext2mime(src)) } );
			
			//set as flash src if is a mp4 file
			if ( ext2mime(src) == 'video/mp4' &&  self.flash_src == '' )
				self.flash_src = src;
		}
		...
		var myContent = '<!DOCTYPE html>'
		    ...
		    + '</style>'
		    + '</head><body><div id="main" class="fill_wnd"><a href="javascript:{}" class="playbtn transition fill_wnd"><i class="transition fill_wnd"></i></a><div class="no_support_msg transition fill_wnd">'+no_support_msg+'</div></div></body></html>';
		...
		//insert HTML5 video tag only if supported or codec available
		if ( self.codec_exists ) {	
			//create iframe video object and copy original video properties
			self.$iframe_vid = $('<video id="video" width="100%" height="100%"'
								+ ( autoplay ? ' autoplay ' : '' )
								+ ( controls ? ' controls ' : '' )
								+ ( loop ? ' loop ' : '' )
								+ ( muted ? ' muted ' : '' )
								+ ( poster ? ' poster="'+poster+'" ' : '' )
								+ ( preload ? ' preload="'+( preload ? 'true' : 'false' )+'" ' : '' )
								//+ ( src ? ' src="'+src+'" ' : '' )
				 				+ ' >'+innerHTML+'</video>').appendTo(self.$iframe_main);		

			
			 
			//bugfix for chrome, force to reload the file
			if ( typeof self.$iframe_vid.get(0).load == 'function' )
				self.$iframe_vid.get(0).load();

			if ( poster ) {
				self.$iframe_vid.get(0).poster = '';
				self.$iframe_vid.get(0).poster = poster;
			}
			...
			if ( autoplay || /firefox/i.test(navigator.userAgent) || 
				 /iPod/i.test(navigator.userAgent) || /iPad/i.test(navigator.userAgent) || /iPhone/i.test(navigator.userAgent) || 
				 /Windows Phone/i.test(navigator.userAgent) || /Windows Phone/i.test(navigator.userAgent) ) {
				;
			} else { 
				...
				self.$playbtn.click(function(){ 
					self.$iframe_vid.get(0).play(); 
					$(this).addClass('playbtn_hide');					 
				});
			}
			...
			//if flash player exists and mp4 source exists show flash player
			if ( self.flash_exists && self.flash_src != '' ) {
			
				var flashvars = 'src='+self.flash_src+'&poster='+poster
				...
				$('<object width="100%" height="100%">'
				 +'<param name="movie" value="'+o3video_config.script_uri+'o3-video.flash.swf'+'"></param>'
				 +'<param name="flashvars" value="'+flashvars+'"></param>'
				 +'<param name="allowFullScreen" value="true"></param>'
				 +'<param name="allowscriptaccess" value="always"></param>'
				 +'<param name="quality" value="hight"></param>'
				 +'<param name="wmode" value="transparent"></param>'
				 +'<embed src="'+o3video_config.script_uri+'o3-video.flash.swf'+'" type="application/x-shockwave-flash" allowscriptaccess="always"'
				 +'wmode="transparent" quality="high" allowfullscreen="true" width="100%" height="100%" flashvars="'+flashvars+'"></embed>'
				 +'</object>').appendTo(self.$iframe_main);	
